...
void Player::moveOrAttack(int dir)
{
	...
	case UP:
		if (row() > 1)
		{
			if (m_arena->numZombiesAt(row() - 1, m_col) > 0)
				m_arena->attackZombieAt(row() - 1, m_col, dir);
			else
				m_row--;
		}
		break;
	...
	case LEFT:
		if (m_col > 1) 
		{
			if (m_arena->numZombiesAt(m_row, m_col - 1))
				m_arena->attackZombieAt(row(), m_col - 1, dir);
			else
				m_col--;
		}
		break;
...
int Arena::numZombiesAt(int r, int c) const
{
	...
	for (int p = 0; p < m_nZombies; p++)
	{
		if ((m_zombies[p]->row() == r) && (m_zombies[p]->col() == c))
			count++;
	}
	return count; 
}
	...
	for (r = 0; r < rows(); r++)
	{
		for (c = 0; c < cols(); c++)
		{
			if (numZombiesAt(r + 1, c + 1) == 1)
				grid[r][c] = 'Z';
			else if (numZombiesAt(r + 1 , c + 1) == 0)
				continue;
			else if (numZombiesAt(r + 1, c + 1) <= 8)
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
	...
	for (int k = 0; k < m_nZombies; k++)
	{
		...
		{
			if (m_zombies[k]->getAttacked(dir) == true)
			{
				delete(m_zombies[k]);
